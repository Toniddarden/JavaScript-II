...
runners.forEach(function(runners) {
    fullName.push(runners.first_name);
    fullName.push(runners.last_name);
})

// ==== Challenge 2: Use .map() ====
// The event director needs to have all the runner's first names converted to uppercase because the director BECAME DRUNK WITH POWER. Convert each first name into all caps and log the result
let allCaps = fullName.map(function(x) { return x.toUpperCase(); });
console.log(allCaps);
...
console.log(`The runners below need a new order of shirts.`)
...
const ticketPriceTotal = runners.reduce((acc, runners) => acc + runners.donation, 0);
...
// Problem 1 && // Problem 2
// change the emails to uppercase AND group them into one list  
let emailList = [];
...
let emailUpperCase = emailList.map(function(x) { return x.toUpperCase(); });
console.log(emailUpperCase);


// Problem 3
// How much more did we recieve in donations after our init goal of $500 which is $100/runner ?


const bigDonationsTotal = runners.reduce((acc, runners) => acc + runners.donation, -500);
console.log(`We recieved ${bigDonationsTotal} over our initial goal!`);
